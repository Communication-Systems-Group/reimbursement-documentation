...
\subsection{Services}
The following sub chapters will explain what interfaces to external service providers are necessary.

\subsubsection{IFI LDAP Server}
The reimbursement-tool fetches user-data from the IFI LDAP server (see figure \ref{fig:context-diagram}) and stores it in the reimbursement-database. By being able to reuse their existing LDAP credentials, users get a comfortable way to login to the system and ensures at the same time that only authenticated users gain access to the system. Furthermore, the user's current roles and the user's manager are assigned automatically and in consequence correctly. For details about how to adjust the LDAP settings please refer to appendix \ref{subsubsec:ldap}.
